...
namespace UnitTestProject1
{
    ...
    public class VictoryTest
    {
        ...
        //Testing CheckDominatingThird method(Used on Victory 12/14) NOT DONE YET
       /* [TestMethod]
        public void TrueCheckDominationThird()
        {
            //Victory 12 - The reach, Vale of arryn, any third one
            //TheValeOfArryn
            //The Reach
            //Test player configurations(Stark)
            //não funciona!
            var mockfamily = new Mock<FamilyViewModel>("Stark");
            Mock<ObjectiveModel> mockObjective = new Mock<ObjectiveModel>(12, "abc");
            PlayerViewModel testPlayer = new PlayerViewModel("player1", mockfamily.Object, mockObjective.Object);
            testPlayer.Id = "1";

            //Creating a mocked region(TheValeOfArryn) and setting it in testPlayer's ownership
            var mockValeKingdom = new Mock<KingdomViewModel>("TheValeOfArryn", 5, 3);
            var mockRegionPosition = new Mock<RegionPosition>("src", 1, 2, 3, 4);
            var mockValeRegion = new RegionViewModel("Vale", mockValeKingdom.Object, mockRegionPosition.Object, 2);
            mockValeRegion.Player = testPlayer;
            mockValeRegion.Player.Id = "1";
            
            //Creating a mocked region(The Reach) and setting it in testPlayer's ownership
            var mockReachKingdom = new Mock<KingdomViewModel>("The Reach", 5, 3);
            var mockReachRegionPosition = new Mock<RegionPosition>("src", 3, 2, 3, 4);
            var mockReachRegion = new RegionViewModel("Reach", mockReachKingdom.Object, mockReachRegionPosition.Object, 2);
            mockValeRegion.Player = testPlayer;
            
            //Creating a mocked region third region(Dorne) and setting it in testPlayer's ownership
            var mockDorneKingdom = new Mock<KingdomViewModel>("Dorne", 5, 3);
            var mockDorneRegionPosition = new Mock<RegionPosition>("src", 2, 2, 3, 4);
            var mockDorneRegion = new RegionViewModel("Dorne", mockDorneKingdom.Object, mockDorneRegionPosition.Object, 2);
            mockDorneRegion.Player = testPlayer;
            mockDorneRegion.Player.Id = "1";

            //Creating the Region List
            List<RegionViewModel> regionList = new List<RegionViewModel>();

            //Setting The Vale of Arryn(6) territories to testPlayer
            for (var i = 0; i < 6; i++)
            {
                regionList.Add(mockValeRegion);
            }

            //Setting The Reach(5) territories to mockPlayer3
            for (var i = 0; i < 5; i++)
            {
                regionList.Add(mockReachRegion);
            }
            ...
            var regionsDominatedByPlayer = regionList.Where(x => x.Player.Id == testPlayer.Id);
            //Act
            var resp = VictoryRule.checkDominationThird(testPlayer,regionList);
